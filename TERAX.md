...
Hosts and groups move between instances through `TransferDialog`, whose three tabs are the only way profiles enter or leave: an OpenSSH config at a location the user chooses, a Terax export from another machine, and the export that writes one. All of the logic is pure in `lib/transfer.ts` and tested; the dialog only renders it.

- **The export is a snapshot of the two stored collections**, versioned by `kind` + `version`. A file from a newer format is refused rather than half-read.
- **Nothing portable may carry a secret.** `parseExport` rebuilds every auth method field by field instead of spreading it, so a `password` or `passphrase` present in a hand-edited or foreign file cannot reach the profile store, which never holds one. The same rebuild is what validates colours through `normalizeAccent` (they reach a style attribute) and clamps ports (they reach `ssh_open`). A file is untrusted input; treat it that way.
- **`background` is dropped on export.** The image lives in this instance's blob store and the id would resolve to nothing on the target machine.
- **Identity is `user@host:port`, and groups match by name.** Two instances generate ids independently, so ids are worthless for deciding whether the incoming "Production" group is the one already here. `replace` keeps the *local* id and `createdAt` so open tabs and workspace scope keys keep resolving; `copy` takes a fresh id and a `uniqueName`.
- **An import is one store write** (`importSnapshot`), because hosts and the groups they land in have to become visible together, or a host would briefly point at a group that does not exist yet and render as orphaned.
- **`lib/transfer.ts` must stay out of the eager graph.** `lib/store.ts` is eager (the opener reaches it so a restored tab can spawn), so importing anything from `transfer.ts` there drags the whole module into the startup chunk; `DEFAULT_SSH_CONFIG_PATH` lives in the store for exactly that reason. `eager-budget.test.ts` locks it.
- **The SSH config location is configurable** and persisted as `sshConfigPath` in `terax-remotes.json`. `ssh_read_config` takes it as an argument and `~`-expands it in Rust. A missing *default* is normal (someone who has never used `ssh`) and returns empty; a missing path the user typed is an error, or a typo would look like an empty config.
