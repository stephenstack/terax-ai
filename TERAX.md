...
- **git runs on the far side** through `run_git`, the single chokepoint for all 19 git commands. It bridges to the async runtime with a channel and `recv_timeout`, the same shape the local path already uses to wait on a child process, rather than making 17 call sites async. `check_git_availability` goes through `run_git` too, or it would report the local git's version for a machine that may not have git.
