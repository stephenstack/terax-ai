...
- **Per-host appearance**: the renderer pool shares one font/cursor/scrollback configuration across its (max 5) slots, so appearance cannot be genuinely per-pane without fragmenting the pool. A session carrying overrides therefore pushes them only while it is the focused pane; a session with none pushes the global values unconditionally, exactly as every local terminal already did. `pruneAppearance` drops any field equal to the global value on save, so editing a host never silently pins whatever the global font happened to be. Types and merge logic live in `terminal/lib/appearanceOverride.ts` (one definition, consumed by both modules).
